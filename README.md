...
点击页面顶部的 **自动演示**，直接在当前界面按 9 个模块、19 段业务流程演示主要功能。可见鼠标会移动、停顿和点击，文本会逐字输入；讲解卡片跟随当前业务区域，同类选项展示代表用法。操作开始时说明文字同步逐字变蓝，按实际动作推进；最后一个操作完成时整段全蓝，短暂停留后进入下一段。操作提示仅在移动、点击、输入等操作时显示，阅读和等待时留空。支持暂停、继续、下一条、倍速、重播和退出。19 段完成后自动展示彩色全流程导图，流动连线串起六个业务阶段与补证复核闭环；点击节点可回看原环节讲解，窄屏纵向展示并就地展开。连线可暂停，并尊重系统减少动态效果偏好。演示文件仍可在导图下方下载。

人才画像分两段演示：先按公司、当前岗位和多种有效证书组合筛选，再设置目标岗位与上岗日期，选择人员并导出取证缺口，最后清空筛选、查看无结果状态及恢复列表。

知识图谱分两段演示：先浏览六类节点全景、缩放和居中，按人员类型与工号定位任职和持证关联；再打开人员详情、查询高压电工作业证、沿关联查看适用规则并进入业务页面，最后演示空结果、清除筛选与回到全景。
