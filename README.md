...
- **人才画像**：搜索和筛选目标岗位，展示资格匹配与取证缺口，导出培训需求。
- **响应式界面**：桌面表格固定关键列，手机端提供摘要卡片和文字导航；支持舒适、紧凑两种表格密度。

## 自动演示

点击页面顶部的 **自动演示**，直接在当前界面按 8 个模块、16 段业务流程演示主要功能。可见鼠标会移动、停顿和点击，文本会逐字输入；讲解卡片跟随当前业务区域，同类选项展示代表用法。结果就位后先停稳，再以 KTV 式逐字变色讲解，读完短暂停留后进入下一段。操作提示仅在移动、点击、输入等操作时显示，阅读和等待时留空。支持暂停、继续、下一条、倍速、重播和退出。16 段完成后自动展示彩色全流程导图，流动连线串起六个业务阶段与补证复核闭环；点击节点可回看原环节讲解，窄屏纵向展示并就地展开。连线可暂停，并尊重系统减少动态效果偏好。演示文件仍可在导图下方下载。
...
目前包含 56 项自动化测试，覆盖证书日期边界、规则匹配、未知状态、导入与归并、整改校验和报表统计等行为。来源核对脚本逐行、逐字段对照岗位工作簿；生产资源检查验证静态托管子路径。

本轮前端验收覆盖 1280px、684px 和 390px 浏览器宽度，以及搜索、选择、详情返回、弹窗、排序和密度切换。具体结果与覆盖边界见[前端验收记录](docs/frontend-update-2026-09-20/验收记录.md)。
