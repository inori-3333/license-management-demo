...
首次打开或刷新文档时，HTML 入口先展示白底居中开场：华电原始图标、海蓝色“启航华电 15 组”、蓝色细载入条。图标裁切样式由 src/brand.css 与 BrandMark 共用，配色和字体消费现有 tokens。图文轻柔上浮，约 1.4 秒后且应用首次提交完成才收满载入条，再淡出进入原页面；条形表示启动过程，不显示模拟百分比。站内导航不重播，减少动态效果时取消动效和刻意等待。启动期间背景不可聚焦；资源加载失败显示重新载入操作。
...
知识图谱是 `/knowledge-graph` 的只读关系探索页，沿用工作台导航与白色控件区；画布复用 flow-background / flow-text / flow-muted，六类实体依次复用 flow-cyan（公司）、flow-mint（专业）、flow-blue（岗位）、flow-violet（人员）、flow-amber（证书）、flow-coral（规则）。颜色区分类别，不表示业务状态。标志性视觉为六组圆形节点环绕中心，弱弹簧回弹、轻微浮动与连线流动；动效可暂停，尊重系统减少动态效果。右侧为节点详情，850px 以下详情自然排列到画布下方，搜索结果移到画布上方；手机点击节点后定位到详情。无额外字体或外部图形资源。

全景每类最多展示 8 个高关联节点，局部每类最多 12 个邻居，手机全景每类 4 个、局部每类 6 个；画布明确标注展示数量与范围，搜索和分页关联列表覆盖全部节点。任职关系按统计日期生效，证书记录显示实际状态，规则关联复用既有评估结果。拖动只改变当前布局，不修改台账；搜索、缩放、选中为临时探索状态，不将人员信息写入 URL 或持久存储。节点使用原生按钮，键盘可选中；缩放、居中与回到全景提供非拖拽替代操作。类别搜索复用 SearchField / Select，详情导航复用现有路由。
